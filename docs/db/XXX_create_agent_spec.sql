-- Migration: XXX_create_agent_spec
...
CREATE TABLE IF NOT EXISTS public.agent_spec (
    ...
    CONSTRAINT chk_observability_provider CHECK (observability_provider IS NULL OR observability_provider IN ('langfuse','langsmith','arize','langwatch','none')),
    CONSTRAINT chk_vector_db_type CHECK (vector_db_type IS NULL OR vector_db_type IN ('lancedb','pgvector','chroma','pinecone','qdrant','milvus','weaviate','redis','none')),
    ...
    CONSTRAINT chk_advanced_observability CHECK (observability_level != 'advanced' OR observability_provider IS NOT NULL),
