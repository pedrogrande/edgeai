...
db/migrations/002_create_agent_spec.sql — CHECK constraints and indexes
...
agent_spec_table = sa.Table(
    ...
    # non_technical | semi_technical | developer | agent | internal
    ...
    sa.Column(
        "memory_type", sa.Text(), nullable=False, server_default=sa.text("'none'")
    ),
    sa.Column(
    ...
    sa.Column(
        "storage_type", sa.Text(), nullable=False, server_default=sa.text("'sqlite'")
    ),
    sa.Column("storage_db_url", sa.Text(), nullable=True),
    # lancedb | pgvector | chroma | pinecone | qdrant | milvus | weaviate | redis
    # NULL = no vector DB needed
    ...
    # none | moderate | deep
    sa.Column(
        "reasoning_level", sa.Text(), nullable=False, server_default=sa.text("'none'")
    ),
    # text | markdown | structured | mixed
    sa.Column(
        "output_format", sa.Text(), nullable=False, server_default=sa.text("'markdown'")
    ),
    ...
    sa.Column(
        "model_provider", sa.Text(), nullable=False, server_default=sa.text("'ollama'")
    ),
    sa.Column(
        "model_id", sa.Text(), nullable=False, server_default=sa.text("'glm-5.1:cloud'")
    ),
    ...
    # langfuse | langsmith | arize | langwatch — NULL means no provider
    ...
    # ── Auto-Derived (filled by derive_spec_metadata, not the user) ────────
